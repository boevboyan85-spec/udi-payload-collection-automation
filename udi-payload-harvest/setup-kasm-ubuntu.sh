...
# AUTO_PUSH (npm run collect): child processes do not inherit exports from this script unless persisted.
# collect.mjs merges ~/.config/udi-payload-harvest/git-push.env into the env for scripts/push-results.sh.
if [[ -n "${GITHUB_TOKEN:-}" ]]; then
  GIT_ENV_DIR="${HOME}/.config/udi-payload-harvest"
  GIT_ENV_FILE="${GIT_ENV_DIR}/git-push.env"
  mkdir -p "$GIT_ENV_DIR"
  umask 077
  {
    echo '# Generated by setup-kasm-ubuntu.sh — chmod 600. Do not commit.'
    printf 'export GITHUB_USERNAME=%q\n' "$GITHUB_USERNAME"
    printf 'export GITHUB_TOKEN=%q\n' "$GITHUB_TOKEN"
  } >"${GIT_ENV_FILE}.new"
  mv "${GIT_ENV_FILE}.new" "$GIT_ENV_FILE"
  chmod 600 "$GIT_ENV_FILE" 2>/dev/null || true
  echo "Wrote ${GIT_ENV_FILE} for non-interactive git push (AUTO_PUSH_RESULTS)."

  BASHRC_MARK="udi-payload-harvest git-push.env"
  touch "$HOME/.bashrc"
  if ! grep -qF "$BASHRC_MARK" "$HOME/.bashrc" 2>/dev/null; then
    {
      echo ""
      echo "# ${BASHRC_MARK} — HTTPS push without prompts (npm run collect)"
      echo '[[ -f "$HOME/.config/udi-payload-harvest/git-push.env" ]] && source "$HOME/.config/udi-payload-harvest/git-push.env"'
    } >>"$HOME/.bashrc"
    echo "Appended source of git-push.env to ~/.bashrc (optional; collect.mjs loads the file directly too)."
  fi
else
  echo "GITHUB_TOKEN unset — AUTO_PUSH will fail over HTTPS until you set GITHUB_TOKEN or re-run setup with a token."
fi
